...
def lambda_handler(event, context):
	...
	if "Records" in event:
        ...
        priceTypeCode = (((((event.get("detail")).get("content")).get("payload")).get("udf")).get("data")[0]).get("priceTypeCode")
        glue_job_data = ((((event.get("detail")).get("content")).get("payload")).get("udf")).get("data")
        if priceTypeCode == "NAV"
            ...
            glue_job_args = {
                "--skipTna": tna_flag,
                "--skipNav": nav_flag,
                "--glueJobData":glue_job_data
            }
            response = trigger_glue_job(glue_job_name, glue_job_args, trigger_key, message_dict)
            
        else:
            print("PriceTypeCode is not NAV")
